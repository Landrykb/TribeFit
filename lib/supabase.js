...
const SUPABASE_URL = cleanUrl(SUPABASE_URL);
const SUPABASE_ANON_KEY = cleanKey(SUPABASE_ANON_KEY);
const SUPABASE_SERVICE_ROLE_KEY = cleanKey(SUPABASE_SERVICE_ROLE_KEY);
...
if (!MOCK_MODE) {
  try {
    ...
    if (SUPABASE_SERVICE_ROLE_KEY && typeof window === 'undefined') {
      realSupabaseAdmin = createClient(
        SUPABASE_URL,
...
export const supabaseAdmin = MOCK_MODE ? mockSupabase : (realSupabaseAdmin || realSupabase);
...
export const adjustWalletTc = async (userId, amountTc, operation = 'add') => {
  ...
  try {
    ...
    // supabase-js v2 has no .raw(); do a read-modify-write
    const { data: current, error: readError } = await client
      .from('users')
      .select('wallet_balance_tc')
      .eq('id', userId)
      .single();
    if (readError) throw readError;

    const balance = current?.wallet_balance_tc || 0;
    const next = operation === 'add' ? balance + amountTc : Math.max(0, balance - amountTc);

    const { data, error } = await client
      .from('users')
      .update({ wallet_balance_tc: next })
      .eq('id', userId)
      .select()
      .single();
...
export const adjustSnatchedTc = async (userId, amountTc, operation = 'add') => {
  ...
  try {
    ...
    const { data: current, error: readError } = await client
      .from('users')
      .select('snatched_balance_tc')
      .eq('id', userId)
      .single();
    if (readError) throw readError;

    const balance = current?.snatched_balance_tc || 0;
    const next = operation === 'add' ? balance + amountTc : Math.max(0, balance - amountTc);

    const { data, error } = await client
      .from('users')
      .update({ snatched_balance_tc: next })
      .eq('id', userId)
      .select()
      .single();
