...
import {
  EVOLUTION_STAGES, getEvolutionStage, stageIndex,
  AVATAR_LOOKS, RESTYLE_FEE_TC, FREE_CHANGES_PER_STAGE,
  AVATAR_BODY_TYPES, DEFAULT_BODY_TYPE, isBodyType, stageArtFor, customStagesFor,
  isLookUnlockedByStage, ownedLooks, lookChangeBudget, sanitizeLook,
} from './avatar-looks';

// Look progression + evolution stages live in lib/avatar-looks.js (pure, unit tested).
export {
  EVOLUTION_STAGES, getEvolutionStage, stageIndex,
  AVATAR_LOOKS, RESTYLE_FEE_TC, FREE_CHANGES_PER_STAGE,
  AVATAR_BODY_TYPES, DEFAULT_BODY_TYPE, isBodyType, stageArtFor, customStagesFor,
...
export async function getAvatarState(userId) {
  ...
  return {
    ...
    body_type: isBodyType(state.body_type) ? state.body_type : DEFAULT_BODY_TYPE,
    custom: null,
    preset: sanitizeLook(state.preset) || null,
    parts: state.parts || {},
    owned_looks: ownedLooks(state, stage.id),
    look_budget: lookChangeBudget(state, stage.id),
...
export async function computeDayStandings(group, dayStr) {
  ...
  for (const uid of Object.keys(byUser)) {
    ...
    standings.push({
      ...
      avatar: { skin: state.skin || 'ember', accessory: state.accessory || 'none', stage: stage.id, body_type: state.body_type || DEFAULT_BODY_TYPE, preset: state.preset || null },
    });
  }
