...
export async function getUser(userId) {
  ...
  if (!data) {
    ...
    return createTestUser('Guest', userId);
  }
  return {
    ...data,
...
export async function addUserToGroup(userId, groupId) {
  let [user, group] = await Promise.all([getUser(userId), getGroup(groupId)]);
  if (!user) throw new Error('User or group not found');
  if (!group) {
    if (!isUuid(groupId)) throw new Error('User or group not found');
    group = await createGroup({ id: groupId, name: 'Founders Tribe', owner_id: userId });
  }
  if (!group) throw new Error('User or group not found');
...
export async function getDailyVersus(groupId) {
  ...
  if (!group) {
    return { day: todayStr, pot: 0, standings: [], last_result: null, member_count: 0, yesterday: yesterdayStr };
  }
