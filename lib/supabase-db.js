...
export async function getUsage(userId) {
  ...
  return data || { streak_shield_month: null, streak_shield_used: 0 };
}
...
export async function saveUsage(userId, usage) {
  ...
  const payload = {
    user_id: userId,
    streak_shield_month: usage.streak_shield_month ?? existing.streak_shield_month,
    streak_shield_used: usage.streak_shield_used ?? existing.streak_shield_used,
    updated_at: nowIso(),
  };
  if (existing?.user_id) {
    await supabaseAdmin.from('user_usage').update(payload).eq('user_id', userId);
  } else {
    await supabaseAdmin.from('user_usage').insert(payload);
  }
  return payload;
}
...
export async function applyStreakShield(userId, capPerMonth = 2) {
  ...
  if (u.streak_shield_month !== key) {
    u.streak_shield_month = key;
    u.streak_shield_used = 0;
  }
  if ((u.streak_shield_used || 0) >= capPerMonth) {
    await saveUsage(userId, u);
    return false;
  }
  u.streak_shield_used = (u.streak_shield_used || 0) + 1;
  await saveUsage(userId, u);
  return true;
}
