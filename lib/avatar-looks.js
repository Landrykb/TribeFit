...
//
// Everyone starts on the evolution artwork for their stage (look = null), which is
// free forever and grows with the user's workouts. Base looks unlock for free as the
// user evolves; recolour variants and premium art cost TribeCoins. Restyling is free
// while the stage allowance lasts, then costs RESTYLE_FEE_TC per change.
...
// Recolour variants are produced by scripts/gen-avatar-variants.mjs and must stay in
// sync with the ids it writes: variant_{slug}_{region}_{color}.svg
const LOOK_FAMILIES = [
  { slug: 'aurora', base: 'preset-6.svg', name: 'Aurora', stage: 'rookie',  price_tc: 60 },
  { slug: 'shadow', base: 'preset-7.svg', name: 'Shadow', stage: 'athlete', price_tc: 80 },
  { slug: 'blaze',  base: 'preset-8.svg', name: 'Blaze',  stage: 'athlete', price_tc: 80 },
];

const VARIANT_REGIONS = [
  { id: 'skin', label: 'Skin', price_tc: 35 },
  { id: 'outfit', label: 'Outfit', price_tc: 25 },
];

const VARIANT_COLORS = ['red', 'orange', 'green', 'cyan', 'blue', 'purple'];
...
function buildLooks() {
  ...
  for (const fam of LOOK_FAMILIES) {
    looks[fam.base] = { id: fam.base, name: `${fam.name} Original`, family: fam.name, stage: fam.stage, price_tc: fam.price_tc };
    for (const region of VARIANT_REGIONS) {
      for (const color of VARIANT_COLORS) {
        const id = `variant_${fam.slug}_${region.id}_${color}.svg`;
        looks[id] = {
          id,
...
export function ownedLooks(state, currentStageId) {
  ...
  const earned = Object.values(AVATAR_LOOKS)
    ...
    .map(l => l.id);
  return Array.from(new Set([...earned, ...stored]));
}
