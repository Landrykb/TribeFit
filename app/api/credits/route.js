...
export async function GET(request) {
  try {
    ...
    const [credits, missed] = await Promise.all([listCatchUpCredits(userId), getMissedWorkouts(userId)]);
...
export async function POST(request) {
  try {
    ...
    if (action === 'consume') {
      const ok = await consumeCatchUpCredit(userId);
      if (!ok) return NextResponse.json({ error: 'No credits available' }, { status: 400 });
      if (body.missedId) {
        try { await removeMissedWorkout(userId, body.missedId); } catch {}
      }
      const [credits, missed] = await Promise.all([listCatchUpCredits(userId), getMissedWorkouts(userId)]);
      return NextResponse.json({ success: true, count: credits.length, credits, missed });
    }
