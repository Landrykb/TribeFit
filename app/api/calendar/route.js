...
export async function GET(request) {
  try {
      ...
      if (scope === 'group' && groupId) {
        try {
          ...
          for (const mid of memberIds) {
            const items = await getCalendarDay({ userId: mid, date });
            items.forEach((it) => all.push({ ...it, user_id: mid }));
          }
          return NextResponse.json({ workouts: all });
        } catch (_) {
          const workouts = await getCalendarDay({ userId, date });
    ...
    const schedule = await listCalendar({ userId });
...
export async function PATCH(request) {
  try {
    ...
    const updated = await updateCalendarEntry({ userId: user_id, id, changes });
...
export async function DELETE(request) {
  try {
    ...
    const ok = await deleteCalendarEntry({ userId: user_id, id });
